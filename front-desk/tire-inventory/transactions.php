<?php
/**
 * Front-Desk Inventory Transaction History
 */
...
function frontdesk_inventory_transaction_url($page, $search, $transaction_type, $date_filter) {
...
<main class="inventory-records-page inventory-transactions-page">
    <header class="inventory-hero">
        <div>
            ...
            <p>Branch stock movement history by date and year</p>
        </div>
        <div class="inventory-hero-actions">
            <a href="/hwtires/front-desk/tire-inventory/" class="inventory-history-btn">
    ...
    <?php if ($total_pages > 1): ?>
        <nav class="records-pagination inventory-records-pagination" aria-label="Inventory transaction pages" style="order: 99; margin-top: 16px; margin-bottom: 24px;">
            <ul class="pagination justify-content-center">
                <?php if ($page > 1): ?>
                    <li class="page-item"><a class="page-link" href="<?php echo esc_attr(frontdesk_inventory_transaction_url(1, $search, $transaction_type, $date_filter)); ?>">First</a></li>
                    <li class="page-item"><a class="page-link" href="<?php echo esc_attr(frontdesk_inventory_transaction_url($page - 1, $search, $transaction_type, $date_filter)); ?>">Previous</a></li>
                <?php endif; ?>
                <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++): ?>
                    <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>"><a class="page-link" href="<?php echo esc_attr(frontdesk_inventory_transaction_url($i, $search, $transaction_type, $date_filter)); ?>"><?php echo (int) $i; ?></a></li>
                <?php endfor; ?>
                <?php if ($page < $total_pages): ?>
                    <li class="page-item"><a class="page-link" href="<?php echo esc_attr(frontdesk_inventory_transaction_url($page + 1, $search, $transaction_type, $date_filter)); ?>">Next</a></li>
                    <li class="page-item"><a class="page-link" href="<?php echo esc_attr(frontdesk_inventory_transaction_url($total_pages, $search, $transaction_type, $date_filter)); ?>">Last</a></li>
                <?php endif; ?>
            </ul>
        </nav>
    <?php endif; ?>
