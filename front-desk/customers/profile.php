<?php
/**
 * Front-Desk Customer Profile & Complete Records
 */
...
if (!function_exists('front_profile_branch_label')) {
    function front_profile_branch_label($name) {
        return app_branch_label($name, '-');
    }
}

if (!function_exists('front_profile_branch_class')) {
    function front_profile_branch_class($branch_id) {
        $branch_id = intval($branch_id);
        return $branch_id > 0 ? 'customer-branch-' . ((($branch_id - 1) % 3) + 1) : 'customer-branch-empty';
    }
}

if (!function_exists('front_profile_vehicle_name')) {
    function front_profile_vehicle_name($record, $include_plate = false) {
...
if (!function_exists('front_profile_short_date')) {
    function front_profile_short_date($date) {
        return !empty($date) ? date('Y-m-d', strtotime($date)) : '-';
    }
}

if (!function_exists('front_profile_money')) {
    function front_profile_money($amount) {
        return '&#8369;' . number_format((float) $amount, 0);
    }
}

if (!function_exists('front_profile_split_services')) {
    function front_profile_split_services($value) {
        if (empty($value)) {
            return [];
        }
...
try {
    ...
    $history_stmt = $pdo->prepare("
        SELECT sh.*, b.name AS branch_name, v.make, v.model, v.plate_number,
               q.quotation_number, jo.job_number
        FROM service_history sh
        LEFT JOIN branches b ON b.id = sh.branch_id
        LEFT JOIN vehicles v ON v.id = sh.vehicle_id
        LEFT JOIN quotations q ON q.id = sh.quotation_id
        LEFT JOIN job_orders jo ON jo.id = sh.job_order_id
    ...
    $approved_stmt = $pdo->prepare("
        ...
        LIMIT 1
    ");
    $approved_stmt->execute([$customer_id, $user_branch_id]);
    $approved_quotation_id = intval($approved_stmt->fetchColumn() ?: 0);
    ...
    error_log('Front customer profile error: ' . $e->getMessage());
    redirect('index.php');
}
...
$create_quote_url = '/hwtires/front-desk/quotations/create.php?customer_id=' . $customer_id;
if (!empty($primary_vehicle['id'])) {
    $create_quote_url .= '&vehicle_id=' . intval($primary_vehicle['id']);
}
?>
...
<div class="customer-profile-page front-customer-profile">
    <a href="/hwtires/front-desk/customers/" class="customer-profile-back">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select vehicle-make-select" name="make" id="profile_add_veh_make" required>
                                <option value="" selected disabled>-- Select Car Brand --</option>
                            </select>
                            <input type="text" class="form-control vehicle-make-custom mt-2" name="make_custom" id="profile_add_veh_make_custom" maxlength="50" data-text-format="first-letter" placeholder="New Vehicle Make * (e.g., Jetour)" style="display: none;">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Model <span class="text-danger">*</span></label>
                            <select class="form-select vehicle-model-select" name="model" id="profile_add_veh_model" required disabled>
                                <option value="" selected disabled>-- Select Brand First --</option>
                            </select>
                            <input type="text" class="form-control vehicle-model-custom mt-2" name="model_custom" id="profile_add_veh_model_custom" maxlength="50" data-text-format="first-letter" placeholder="New Vehicle Model * (e.g., X70)" style="display: none;">
                        </div>
                    </div>
